...
public class Aims {
	...
	public  void selectInMenu() {
		...
		switch(selection) {
			case 0: {
				...
				System.exit(0);
				break;}
			case 1: // display all the items in the store and go to storeMenu
			{	
				store.available();
				selectInStoreMenu();
				break;
			...
			case 2: // allow the user to add a media to or remove a media from the store
			{
				...
				if(select.compareTo("r")==0) {
					...
					System.out.println("Enter the type of media: CD, DVD or a book title");
	...
	public void selectInStoreMenu() {
		...
		int selection = Integer.valueOf(System.console().readLine());
		switch(selection) {
			case 0: selectInMenu(); break; // go back
			case 1:  //See a media's details
			{
				...
				if(x!=null) {
					...
					System.out.println("Can not found, please enter again!");
					selectInStoreMenu();
				}
				break;
			}
			...
			case 3: //Play a media
			{
				...
				if (x != null){
					if(x instanceof DigitalVideoDisc) {
						DigitalVideoDisc xDVD = (DigitalVideoDisc)x;
						xDVD.play();
					}
					else if(x instanceof CompactDisc) {
						CompactDisc xCD = (CompactDisc)x;
						xCD.play();
					}
	...
	public void selectInMediaDetailsMenu(Media x) {
		...
		int selection = Integer.valueOf(System.console().readLine());
		switch(selection) {
			case 0: selectInStoreMenu(); break; // go back
			case 1: 
			//add to cart
			...
			{if(x instanceof DigitalVideoDisc) {
				...
				{System.out.println("Book cannot play, please try again!");}
			selectInStoreMenu();
			break;
			}
			default:{
	...
	public  void selectInCartMenu() {
		...
		switch(select) {
			...
			case 5: { 
				//place order and empty current cart
	...
	public static void main(String[] args) {
		Book firstBook = new Book("Calculus 1");
		Book secondBook = new Book("Algebra");
		Cart cart = new Cart();
		Store store = new Store();
		store.addMedia(firstBook);
		store.addMedia(secondBook);
